...
const resolutionMap={1:"1",3:"3",5:"5",15:"15",60:"60","1D":"1D"};
...
async function get15mMove(pair){
  try{
    ...
    const from=now-15*60-90;
    const j=await (await fetch(`/api/candles?pair=${encodeURIComponent(pair)}&resolution=15&from=${from}&to=${now}`)).json();
    const c=(j.data||[]).slice().sort((a,b)=>a.time-b.time);
    if(c.length<2)return null;
    const first=Number(c[c.length-2]?.close);
...
async function scanVisible(){
  ...
  $("scanStatus").textContent="15m movement scan चल रहा है…";

  const scored=[];
  const batch=8;
  for(let i=0;i<pairs.length;i+=batch){
    const part=pairs.slice(i,i+batch);
    const vals=await Promise.all(part.map(async p=>({p,m:await get15mMove(p)})));
    vals.forEach(x=>{if(Number.isFinite(x.m))scored.push(x)});
    if(i%80===0) render();
  }

  scored.sort((a,b)=>Math.abs(b.m)-Math.abs(a.m));
  const top=scored.slice(0,60).map(x=>x.p);

  $("scanStatus").textContent=`15m movers: ${scored.length} • RSI scan: ${top.length}`;
...
$("refresh").onclick=async()=>{await loadPrices();await scanVisible()};
